...
<%
    ...
    ArrayList lista_sesion_quimico = neg.lista_sesion_quimico(id_duo);
...
<table>
    <tr>
        <td rowspan="2" ></td>
        <td <%=titulo%> Datos del Químico Farmacéutico</td>
    </tr>
    <tr>
...
<table>
    <tr valign="top" >
        <td>
            <fieldset>
                <legend>Sesión Químico Farmacéutico</legend>
                <table>
                    <tr>
                        <%    //
                            Iterator it_lista_sesion_psicolo = lista_sesion_quimico.iterator();
                            int contador = 0;
                            if (lista_sesion_quimico.size() == 0) {
                                out.write("<h3>Sin registro</h3>");
                            } else if (it_lista_sesion_psicolo.hasNext()) {
                                cSesionKine ses = (cSesionKine) it_lista_sesion_psicolo.next();
                                contador++;

                                out.write("<tr><td>");
                                out.write("<form  action='../PDF_sesion_quimico' method='POST' target='_blank' > "
...
<fieldset>
    ...
    <center>
        <TABLE>
            <TR>
                <TD>
                    <form name="form_kin"  action="sesion_quimico.jsp" method="POST"  >
                        <input type="hidden" name="txt_duo" value="<%=id_duo%>" > 
                        <input class="btn btn-primary" type="submit" value="Registrar Sesión de Quimico Farmaceutico" name="btn_ses" />
                    </form> 
                </TD>
