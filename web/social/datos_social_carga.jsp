...
<%
    ...
    ArrayList lista_sesion_tera = neg.lista_sesion_social(id_duo);
...
<table>
    <tr>
        <td rowspan="2" ><img src="../Imagenes/kinesiologia.png" width="48" height="48" alt="Trabajadora Social"/></td>
        <td <%=titulo%> Datos del Trabajadora Social Ocupacional</td>
    </tr>
    <tr>
...
<table>
    <tr valign="top" >
        <td>
            <fieldset>
                <legend>Sesión Trabajadora Social</legend>
                <table>
                    <tr>
                        <%    //
                            Iterator it_lista_sesion_terapeuta= lista_sesion_tera.iterator();
                            int contador = 0;
                            if (lista_sesion_tera.size() == 0) {
                                out.write("<h3>Sin registro</h3>");
                            } else {
                                if (it_lista_sesion_terapeuta.hasNext()) {
                                    cSesionKine ses = (cSesionKine) it_lista_sesion_terapeuta.next();
                                    contador++;

                                    out.write("<tr><td>");
                                    out.write("<form  action='../PDF_sesion_social' method='POST' target='_blank' > "
...
<fieldset>
    ...
    <center>
        <TABLE>
            <TR>
                <TD>
                    <form name="form_kin"  action="sesion_social.jsp" method="POST"  >
                        <input type="hidden" name="txt_duo" value="<%=id_duo%>" > 
                        <input class="btn btn-primary" type="submit" value="Registrar Sesión Trabajadora Social" name="btn_ses" />
                    </form> 
                </TD>
