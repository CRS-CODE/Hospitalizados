        ...
        switch (opcion) {
            case 1:
                ...
                if (!obtiene_usuario.equalsIgnoreCase("null")) {
                    ...
                    String[] ids = request.getParameterValues("idDiagnostico");
                    String[] descripciones = request.getParameterValues("descripcionDiagnostico");

                    for (int i = 0; i < ids.length; i++) {
                        int id = Integer.parseInt(ids[i]);
                        String descripcion = descripciones[i];
                        neg.actualizarDiagnostico(id, descripcion);
                    }
                } else {
