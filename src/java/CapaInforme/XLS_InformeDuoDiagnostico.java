 ...
 * @author Silvio
 */
public class XLS_InformeDuoDiagnostico extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        ArrayList lista = neg.obtiene_duo_liviano_x_fecha();
        ArrayList lista_diag = neg.lista_diagnostico_segun_duo();
        ...
        try {
            ...
            cell.setCellValue("NÓMINA DE DIAGNOSTICOS"); //vacia el nombre de las columnas en la hoja de excell
            ...
            cell.setCellValue("Fecha seleccionada    :  Entre "); //vacia el nombre de las columnas en la hoja de excell
            ...
            while (it.hasNext()) {
                cDuo das = (cDuo) it.next();
                control_fila++;

                row = (HSSFRow) sheet0.createRow((short) control_fila); //crea el renglon
                cell = row.createCell((short) 0); //crea la celda tipo
                cell.setCellValue(das.getId_duo());
                cell.setCellStyle(style_Normal);

                cell = row.createCell((short) 1); //crea la celda tipo
                cell.setCellValue(das.getFecha_hora_ing_duo());
                cell.setCellStyle(style_Normal);
                ...
                Iterator it1 = lista_diag.iterator();
                while (it1.hasNext()) {
                    cDiagnostico diag = (cDiagnostico) it1.next();
                    if (das.getId_duo() == diag.getId_duo()) {
                        diagnosticos += "||" + diag.getDescripcion_diagnostico() + " ";
                    }
                }
